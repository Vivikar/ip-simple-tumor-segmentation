...
def linear_transform_scaling(
    image: np.ndarray, q25_map=600, q75_map=800, min_value=200, max_value=1200
):
    image = image.copy()
...
def retrieve_brats_data(
    patients, modalities, brats_path="", normalize=True, dataset_part="_Training_"
):
    ...
    for patient_idx, patient in enumerate(patients):
        if patient < 10:
            brain_path = (
                f"BraTS20{dataset_part}00{patient}/BraTS20{dataset_part}00{patient}"
            )
            segm_path = f"BraTS20{dataset_part}00{patient}/BraTS20{dataset_part}00{patient}_seg.nii"
        elif 10 <= patient < 100:
            brain_path = (
                f"BraTS20{dataset_part}0{patient}/BraTS20{dataset_part}0{patient}"
            )
            segm_path = f"BraTS20{dataset_part}0{patient}/BraTS20{dataset_part}0{patient}_seg.nii"

        else:
            brain_path = (
                f"BraTS20{dataset_part}{patient}/BraTS20{dataset_part}{patient}"
            )
            segm_path = (
                f"BraTS20{dataset_part}{patient}/BraTS20{dataset_part}{patient}_seg.nii"
            )
...
def kurtosis_filter(img, size):
    imgS = img * img
    m, m2p, m3p, m4p = (
        uniform_filter(x, size) for x in (img, imgS, imgS * img, imgS * imgS)
    )
    mS = m * m
...
def feature_extractor_1(patient_scans, bx_idxs, by_idxs, bz_idxs, kernel_size=3):
    ...
    X = np.stack(
        [
            patient_scans[i][(bx_idxs, by_idxs, bz_idxs)]
            for i in range(1, len(patient_scans))
        ]
    ).T
    ...
    mean_f = np.stack(
        (
            uniform_filter(patient_scans[i], size=kernel_size,)[
                (bx_idxs, by_idxs, bz_idxs)
            ]
            for i in range(1, len(patient_scans))
        )
    ).T
    var_f = np.stack(
        (
            variance_filter(patient_scans[i], size=kernel_size,)[
                (bx_idxs, by_idxs, bz_idxs)
            ]
            for i in range(1, len(patient_scans))
        )
    ...
    skew_f = np.stack(
        (
            skeweness_filter(patient_scans[i], size=kernel_size,)[
                (bx_idxs, by_idxs, bz_idxs)
            ]
            for i in range(1, len(patient_scans))
        )
    ...
    kurtosis_f = np.stack(
        (
            kurtosis_filter(patient_scans[i], size=kernel_size,)[
                (bx_idxs, by_idxs, bz_idxs)
            ]
            for i in range(1, len(patient_scans))
        )
...
def evaluate_brain(
    patient_scans, feature_extraction_func, model, sample=False, kernel_size=3
):
    ...
    """
    brain_mask = (
        (patient_scans[1] != 0) & (patient_scans[2] != 0) & (patient_scans[3] != 0)
    )
    bx_idxs, by_idxs, bz_idxs = np.where((brain_mask != 0).astype(bool))

    if sample:
        sampled_tumor_points = np.random.randint(low=0, high=len(bx_idxs), size=sample)
    ...
    if not sample:
        ...
        for b_idx in tqdm(range(len(bx_idxs))):
            predicted_mask[
                bx_idxs[b_idx], by_idxs[b_idx], bz_idxs[b_idx]
            ] = y_predicted[b_idx]
...
def calculate_metrics(patient_data, predicted_mask):
    ...
    print(
        "DSC for all tumor regions: ",
        f1_score(patient_data[0, 0][brain_mask] > 0, predicted_mask[brain_mask] > 0),
    )


def plot_results(patient_data, predicted_mask, slice):
    ...
    for mri_mod in range(4):
        ax[mri_mod, 0].imshow(patient_data[0, mri_mod + 1][:, :, slice], cmap="gray")
        ax[mri_mod, 0].set_title(modality_names[mri_mod] + f" {slice} slice")
        ax[mri_mod, 0].axis("off")

    ax[0, 1].imshow(patient_data[0, 0][:, :, slice], cmap="gray")
    ...
    ax[2, 1].imshow(patient_data[0, 0][:, :, slice] > 0, cmap="gray")
    ax[2, 1].imshow(predicted_mask[:, :, slice] > 0, cmap="gray", alpha=0.5)
    ax[2, 1].set_title("Predicted over Original Mask")
    ax[2, 1].axis("off")

    ax[3, 1].imshow(patient_data[0, 3][:, :, slice], cmap="gray")
    ax[3, 1].imshow(predicted_mask[:, :, slice] > 0, cmap="gray", alpha=0.5)
    ax[3, 1].set_title("Predicted mask over Flair")
    ax[3, 1].axis("off")
...
def plot_results_for_validation(patient_data, predicted_mask, slice):
    """Plots results of the segmentation along with 
    patient scans in different modalities

    Args:
        patient_data (np.ndarray(4,x_dim,y_dim,z_dim)): Patients MRI scans and segmentation mask in the order
        predicted_mask (np.ndarray(x_dim, y_dim, z_dim)): Predicted 3D volume of segmentation
        slice (int): Slice number
    """
    fig, ax = plt.subplots(4, 2, figsize=(12, 12))
    ...
    for mri_mod in range(4):
        ...
        ax[mri_mod, 1].imshow(patient_data[0, mri_mod + 1][:, :, slice], cmap="gray")
        ax[mri_mod, 1].imshow(predicted_mask[:, :, slice], alpha=0.5)
        ax[mri_mod, 1].set_title(
            modality_names[mri_mod] + f" {slice} slice " + "predicted mask overlay"
        )
        ax[mri_mod, 1].axis("off")
...
def global_slice_thresholding(test_slice, k=3, max_tumor_prc=0.35):
    ...
    otsu_mask = (test_slice > threshold_otsu(test_slice[test_slice != 0])).astype(int)
    otsu_mask[test_slice == 0] = 0

    # Check if region is foreground or tumor
    isodata_mask = (test_slice > threshold_isodata(test_slice[test_slice != 0])).astype(
        int
    )
...
def threshold_patient(patient_data, modality=4):
    ...
    for slice_idx in range(155):
        if (
            patient_data[0, modality, :, :, slice_idx] > 0
        ).sum() > 1:  # Not empty slice

            otsu_mask_morph, isodata_mask_morph = global_slice_thresholding(
                (patient_data[0, modality, :, :, slice_idx])
            )
        else:
...
def extract_features_kmeans(patient_scans, slice):
    X = np.stack(
        [patient_scans[i][:, :, slice].flatten() for i in range(1, len(patient_scans))]
    ).T
    return X
...
def two_roundest_segmented_regions(model, slice_features):
    ...
    circularity_scores = [
        np.abs(
            1
            - perimeter(final_mask == cls) ** 2
            / (4 * np.pi * (final_mask == cls).sum())
        )
        for cls in range(5)
    ]
    return [
        (final_mask == tumor_cand).astype(int)
        for tumor_cand in np.argsort(circularity_scores)[:2]
    ]
...
def evaluate_patient_KMeans(model, patient_data, max_tumor_prc=0.34):
    ...
    for slice_idx in range(patient_data[0, 0].shape[-1]):
        ...
        for predicted_tumor_slice_cand in two_roundest_segmented_regions(
            model, slice_features
        ):
            # check if candidate region is not in the background and has a meaningfull size
            if (
                (
                    (patient_data[0, 1, :, :, slice_idx] == 0)
                    & (predicted_tumor_slice_cand)
                ).sum()
                == 0
                and predicted_tumor_slice_cand.sum()
                / ((patient_data[0, 1, :, :, slice_idx] > 0).sum())
                < max_tumor_prc
            ):
                predicted_tumor_slice = predicted_tumor_slice_cand

        predicted_tumor_mask[:, :, slice_idx] = predicted_tumor_slice
    return binary_closing(binary_opening(predicted_tumor_mask))


def display_results(patient_data, predicted_mask, slice, patient_no, dataset_part):
    """Plots results of the segmentation along with 
    patient scans in different modalities
    ...
    fig.tight_layout()
    for mri_mod in range(4):
        ax[0, mri_mod].imshow(patient_data[0, mri_mod + 1][:, :, slice], cmap="gray")
...
#     """
#     test_arr_2, true_TM_px1_BM, vox_idx, original_mask, patient_data = obtain_test_data(
#         patient_number, dataset_part, brats_path
#     )
...
#         pred_TM_px1_final[
#             vox_idx[pred_idx][0], vox_idx[pred_idx][1], vox_idx[pred_idx][2]
#         ] = pred
...
#         px_ID = [patients[i]]
#         study = retrieve_brats_data_rf(
#             px_ID,
#             modality_names,
#             normalize=False,
#             dataset_part=dataset_path,
...
#                 study[0, 2, :, :, :], size=(3, 3, 3), mode="constant"
#             )
#         )
#         # T1 min 3x3x3
#         study_FE[0].append(
#             ndimage.minimum_filter(
#                 study[0, 1, :, :, :], size=(3, 3, 3), mode="constant"
#             )
#         )
#         # FLAIR min 3x3x3
#         study_FE[0].append(
#             ndimage.minimum_filter(
#                 study[0, 3, :, :, :], size=(3, 3, 3), mode="constant"
#             )
#         )
#         # T1CE min 3x3x3
#         study_FE[0].append(
#             ndimage.minimum_filter(
#                 study[0, 4, :, :, :], size=(3, 3, 3), mode="constant"
#             )
#         )
#         # T2 max 3x3x3
#         study_FE[0].append(
...
#                 study[0, 1, :, :, :], size=(11, 11, 11), mode="constant"
#             )
#         )
#         # T1 median 11x11
...
#         data_set = np.zeros(
#             (np.shape(brain_mask)[1], no_features - 1)
#         )  # create data set of voxels of 1 patient
#         # print('data_set size: ',np.shape(data_set))
#         for j in range(1, no_features):
#             data_set[:, j - 1] = study_FE[0][j][
#                 brain_mask
#             ]  # fill each column with reduced number of voxels (only brain)
...
#             study_filtered[:, :, k] = ndimage.median_filter(
#                 study[0, 1, :, :, k], size=(11, 11), mode="constant"
#             )
#     else:
#         for k in range(np.shape(study)[-1]):
#             # study_filtered[:,:,k] = ndimage.median_filter(study[:,:,k],size=(11,11),mode='constant')
#             study_filtered[:, :, k] = ndimage.median_filter(
#                 study[0, :, :, k], size=(11, 11), mode="constant"
#             )
...
#             study_filtered[:, :, k] = ndimage.convolve(
#                 study[0, 3, :, :, k], kernel, mode="constant"
#             )
#     else:
#         for k in range(np.shape(study)[-1]):
#             # study_filtered[:,:,k] = ndimage.convolve(study[:,:,k],kernel,mode='constant')
#             study_filtered[:, :, k] = ndimage.convolve(
#                 study[2, :, :, k], kernel, mode="constant"
#             )
#     return study_filtered


# def retrieve_brats_data_rf(
#     patients, modalities, brats_path="", normalize=True, dataset_part="_Training_"
# ):
#     """Retrieves BRATS2020 data for given patients of given MR modalities
